...
if(isset($_GET["pdf"]) && isset($_GET['order_id']))
{
	...
	foreach($result as $row)
	{
		...
		';
		$statement = $connect->prepare("
			SELECT inventory_order_product.*, product.product_name, unit.unit_name
			FROM inventory_order_product 
			JOIN product ON inventory_order_product.product_id = product.product_id 
